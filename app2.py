...
loaded_model = pickle.load(open("model7.pkl", "rb"))
...
def reset():
    ...
    symptoms_dict[request.form["add"]] = 15
    ...
    cols.insert(-1,tmp)
    df = df[cols]
    df.to_csv("data/clean/dog_disease_train.csv", mode ='w', encoding='cp949',index=None)
    ...
    df.to_csv("data/clean/dog_disease_test.csv", mode ='w', encoding='cp949',index=None)
...
def home():
    global symp
    global _symp
    input_vector = np.zeros(len(symptoms_dict)) #외부값 이므로 안바뀐다.

    # print("form:  ",request.form)
    # print("from 길이:", len(request.form))
    #
    # print("symp를 비게할경우 상태", symp )
    for i in range(len(request.form)):
        # k = i+1;
        symp.append(request.form["증상"+str(i)]) #value값 저장
    print("symp값 :  ",symp)
    if(len(request.form)!=0): # 세션이비어있을경우 값을 추가하여 넣어준다
        for i, j in enumerate(symp):
            if(symp[i] !=''):
                _symp.append(symptoms_dict[j]) # 병명 저장

        input_vector[_symp] = 1 # 해당인덱스 1로표시

        return jsonify({
            "data": loaded_model.predict([input_vector])[0],
            "기관지확장증": loaded_model.predict_proba([input_vector])[0][0],
            "마카다미아너트중독증": loaded_model.predict_proba([input_vector])[0][1],
            "코로나바이러스": loaded_model.predict_proba([input_vector])[0][2],
            })
